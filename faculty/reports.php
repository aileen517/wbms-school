            ...
            <div class="row">
              <div class="col-md-12">
                <div class="card bg-white">
                          ...
                          <div class="input-group">
                            <select name="qsection" class="custom-select" id="inputGroupSelect04" aria-label="Example select with button addon">
                              ...
                              <?php 
                                ...
                                $filter_schedule = mysqli_query($conn, "SELECT * FROM $schedule WHERE faculty='".$_SESSION['SESS_FACULTY_ID']."' AND status='open' ORDER BY section_id ASC");
                  ...
                  <button type="button" class="btn btn-primary  col-sm-5" onclick='printtag("DivIdToPrint");'/><i class="nav-icon fa fa-print"></i> Print Schedule</button></div>
                  <div class="card-body">
                    <div id='DivIdToPrint'>
                      <div class="row">
                        <h4 class="text-center col-md-12 font-weight-bold text-uppercase" >
                          <?php 
                            include("db_connect.php");
                            $q_faculty = mysqli_query($conn, "SELECT * FROM $faculty WHERE facode='".$_SESSION['SESS_FACULTY_ID']."'");
                            while($row_faculty = mysqli_fetch_array($q_faculty)) {
                              echo $row_faculty['fname']." ".$row_faculty['lname'];
                            }
                            mysqli_close($conn);
                          ?>
                        </h4>
                            ...
                            <tbody>
                              <?php
                                if(isset($_POST['FilterSchedule'])) {
                                  ...
                                  $display_schedule = mysqli_query($conn, "SELECT * FROM $schedule WHERE faculty='".$_SESSION['SESS_FACULTY_ID']."' AND status='open' AND section_id='".$_POST['qsection']."' ORDER BY section_id ASC");
                                  ...
                                  include("db_connect.php");
                                  date_default_timezone_set("Asia/Manila");
                                  $display_schedule = mysqli_query($conn, "SELECT * FROM $schedule WHERE faculty='".$_SESSION['SESS_FACULTY_ID']."' AND status='open' ORDER BY section_id ASC");
                                  if(mysqli_num_rows($display_schedule)<1) {
                                    echo "<tr>";
                                      echo "<td colspan='5'>No schedule found.</td>";
                                    echo "</tr>";
