            ...
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
              ...
              <li class="nav-item"><a href="#" class="nav-link text-uppercase font-weight-bold shadow-sm p-3 active"><i class="nav-icon fa fa-cogs"></i><p> Settings<i class="fas fa-angle-left right"></i></p></a>
                <ul class="nav nav-treeview">
                  ...
                  <li class="nav-item"><a href="faculty" class="nav-link text-uppercase font-weight-bold shadow-sm p-3 active"><i class="fa fa-bars nav-icon"></i><p> Faculty</p></a></li>
              ...
              <div class="col-md-12">
                <div class="card bg-white">
                  <div class="card-header">
                    <h5 class="card-title text-uppercase font-weight-bold"><i class="nav-icon fa fa-bars"></i> Faculty List</h5>
                  </div>
                  <div class="card-body">
                    <div class="row">
                      <a href='#' data-toggle="modal" data-target="#addFaculty" class="mb-2 text-uppercase"><span class='badge bg-info rounded p-2'><i class="fas fa-user-plus"></i> Add New Faculty</span></a>
                      <div class="table-responsive">
                        <table id="zero_config" class="table table-striped h6">
                          ...
                          <tbody>
                            <?php
                              include("db_connect.php");
                              $display_faculty = mysqli_query($conn, "SELECT * FROM $faculty ORDER BY fname ASC");
                              if(mysqli_num_rows($display_faculty)<1) {
                                echo "<tr>";
                                  echo "<td colspan='8'>No record found.</td>";
                                echo "</tr>";
                              } 
                              else {
                                while($row_faculty = mysqli_fetch_array($display_faculty)) {
                                  echo "<tr>";
                                    echo "<td><span id='fname".$row_faculty['facID']."'>".ucfirst($row_faculty['fname'])."</span></td>";
                                    echo "<td><span id='lname".$row_faculty['facID']."'>".ucfirst($row_faculty['lname'])."</span></td>";
                                    echo "<td><span id='title".$row_faculty['facID']."'>".ucfirst($row_faculty['title'])."</span></td>";
                                    echo "<td><span id='educ".$row_faculty['facID']."'>".ucfirst($row_faculty['educ'])."</span></td>";
                                    echo "<td><span id='ofc".$row_faculty['facID']."'>".ucfirst($row_faculty['ofc'])."</span></td>";
                                    echo "<td><span id='addr".$row_faculty['facID']."'>".ucfirst($row_faculty['addr'])."</span></td>";
                                    echo "<td><span id='con".$row_faculty['facID']."'>".ucfirst($row_faculty['con'])."</span></td>";
                                    echo "<td><span id='email".$row_faculty['facID']."'>".ucfirst($row_faculty['status'])."</span></td>";
                                    echo "<td>";
                                      if($row_faculty['status']=="active") {
                                        echo "<button type='button' class='btn btn-success rounded-pill btn-xs editfclty text-white' value='".$row_faculty['facID']."'><i class='fas fa-pencil-alt text-warning p-1'></i> Edit</button> <a href='?deactivate_faculty=".$row_faculty['facID']."'><span class='badge bg-danger rounded-pill'><i class='fas fa-times text-warning p-1'></i> Deactivate</span></a>";
                                      }
                                      else if($row_faculty['status']=="inactive") {
                                        echo "<button type='button' class='btn btn-success rounded-pill btn-xs editfclty text-white' value='".$row_faculty['facID']."'><i class='fas fa-pencil-alt text-warning p-1'></i> Edit</button> <a href='?activate_faculty=".$row_faculty['facID']."'><span class='badge bg-primary rounded-pill'><i class='fas fa-check text-warning p-1'></i> Activate</span></a>";
                                      }
                                    echo "</td>";
                                  echo "</tr>";
                                }
