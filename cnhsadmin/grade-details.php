              ...
              <div class="col-md-12">
                <div class="card bg-white">
                  <div class="card-header">
                    <h5 class="card-title text-uppercase font-weight-bold"><i class="nav-icon fa fa-bars"></i> Grade <?php echo $_GET['level'];?> Details</h5>
                  </div>
                  <div class="card-body">
                    <div class="row">
                        <form class="form col-md-12 h6" method="post" action="" enctype="multipart/form-data">
                            <?php
                                include("db_connect.php");
                                $procedure = mysqli_query($conn, "SELECT * FROM $message WHERE type='".$_GET['level']."'");
                                if(mysqli_num_rows($procedure)>0) {
                                    while($row_proced = mysqli_fetch_array($procedure)) {
                            ?>
                                        <div class="mb-3 row">
                                            <label for="details" class="col-sm-3 text-end control-label col-form-label">Details : </label>
                                            <div class="col-lg-12">
                                                <textarea name="details" id="details" cols="300" rows="15" class="form-control" style="resize: none;"><?php echo $row_proced['details'];?></textarea>
                                                <input type='hidden' name="gid" value="<?php echo $_GET['level'];?>">
                                            </div>
                                        </div>
                                        <div class="mb-3 row">
                                          <button type="submit" name="UpdateProcedure" class="btn btn-primary"><i class="fa fa-plus-square" aria-hidden="true"></i> Update Procedure</button>
                                        </div>
                            <?php
                                    }
                                }
                            ...
                            ?>
                                    ...
                                    <div class="mb-3 row">
                                      <button type="submit" name="SubmitProcedure" class="btn btn-primary"><i class="fa fa-plus-square" aria-hidden="true"></i> Add Procedure</button>
                                    </div>
                            <?php
                                mysqli_close($conn);
                                }
