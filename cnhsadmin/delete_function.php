<?php
	if(isset($_GET['action'])){
		...
		if($action == 'delete_level'){
			...
			if($conn->query($sql)){
			    mysqli_query($conn, "DELETE FROM $section WHERE glevel='$id'");
			    $sec_query = mysqli_query($conn, "SELECT * FROM $section WHERE glevel='$id'");
			    while($row_sec = mysqli_fetch_array($sec_query)) {
			        mysqli_query($conn, "DELETE FROM $schedule WHERE section_id='".$row_sec['secID']."'");
