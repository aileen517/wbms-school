...
<?php
	...
	if(!empty($_POST)) {
		...
		$check_registrar = mysqli_query($conn, "SELECT * FROM $registrar WHERE fname='$fname' AND lname='$lname'");
		if(mysqli_num_rows($check_registrar)>0) {
			...
			$insert_registrar = mysqli_query($conn, "INSERT INTO $registrar(regcode,dateentry,fname,lname,title,educ,ofc,email,password,con,addr,status)VALUES('$user_code', '$datenow', '$fname', '$lname', '$title', '$educ', '$ofc', '$email', '$passw', '$con', '$addr', 'active')");
	    	if($insert_registrar) {
	    		...
	    		echo "<div></div><script>Swal.fire({
	  				...
	  				icon: 'error',
	  				title: 'Error',
	  				text: 'New registrar was not successfully added',
	  				showConfirmButton: false,
	  				timer: 1500
				})</script>";
	    	}
