...
<html lang="en">
  ...
  <body class="hold-transition dark-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
          ...
          <nav class="mt-2">
            <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
              ...
              <li class="nav-item"><a href="#" class="nav-link text-uppercase font-weight-bold shadow-sm p-3 active"><i class="nav-icon fa fa-cogs"></i><p> Settings<i class="fas fa-angle-left right"></i></p></a>
                <ul class="nav nav-treeview">
                  <li class="nav-item"><a href="school-year" class="nav-link text-uppercase font-weight-bold shadow-sm p-3 active"><i class="fa fa-bars nav-icon"></i><p> School Year</p></a></li>
              ...
              <div class="col-md-12">
                <div class="card bg-white">
                  <div class="card-header">
                    <h5 class="card-title text-uppercase font-weight-bold"><i class="nav-icon fa fa-bars"></i> School Year</h5>
                  </div>
                  <div class="card-body">
                    <div class="row">
                      <form class="col-md-6 h6" method="post" action="">
                        <?php
                          if(isset($_GET['update'])) {
                            include("db_connect.php");
                            $q_syear = mysqli_query($conn, "SELECT * FROM $schoolyear WHERE syID='".$_GET['update']."'");
                            while($row_syear = mysqli_fetch_array($q_syear)) {
                        ?>
                              <div class="mb-3 row">
                                <label for="glevel" class="col-sm-3 text-end control-label col-form-label">School Year : </label>
                                <div class="col-sm-9">
                                  <input type="text" name="syear" class="form-control fs-5" id="syear" value="<?php echo $row_syear['syear'];?>" required />
                                  <input type="hidden" name="syid" class="form-control fs-5" id="syid" value="<?php echo $row_syear['syID'];?>" required />
                                </div>
                              </div>
                             <div class="mb-3 row">
                                <button type="submit" name="Updatesyear" class="btn btn-primary"><i class="fa fa-check" aria-hidden="true"></i> Update School Year</button>
                              </div>
                        <?php
                              mysqli_close($conn);
                            }
                        ...
                        ?>
                            <div class="card-body">
                              <div class="mb-3 row">
                                ...
                                <div class="col-sm-9">
                                  <input type="text" name="syear" class="form-control fs-5" id="syear" required />
                                </div>
                              </div>
                            <div class="mb-3 row">
                              <button type="submit" name="Submitsyear" class="btn btn-primary"><i class="fa fa-plus-square" aria-hidden="true"></i> Add School Year</button>
                            </div>
                        <?php
                          } 
                        ?>
                      ...
                      <div class="table-responsive">
                        <table id="zero_config" class="table table-striped h6">
                          ...
                          <tbody>
                            <?php
                              include("db_connect.php");
                              $display_syear = mysqli_query($conn, "SELECT * FROM $schoolyear ORDER BY syear ASC");
                              if(mysqli_num_rows($display_syear)<1) {
                                echo "<tr>";
                                  echo "<td colspan='3'>No record found.</td>";
                                echo "</tr>";
                              } 
                              else {
                                while($row_syear = mysqli_fetch_array($display_syear)) {
                                  echo "<tr>";
                                    echo "<td>".$row_syear['syear']."</td>";
                                    echo "<td>".ucfirst($row_syear['status'])."</td>";
                                    echo "<td>";
                                      if($row_syear['status']=="deactivated") {
                                        echo "<a href='?update=".$row_syear['syID']."'><span class='badge bg-success rounded-pill'><i class='fas fa-pencil-alt text-warning p-1'></i> Edit</span></a> <a href='#' class='delete_syear' data-id='".$row_syear['syID']."'><span class='badge bg-danger rounded-pill'><i class='fas fa-trash text-warning p-1'></i> Delete</span></a> <a href='?activate=".$row_syear['syID']."'><span class='badge bg-info rounded-pill'><i class='fas fa-check text-white p-1'></i> Activate</span></a>";
                                      }
                                      else {
                                        echo "<a href='?update=".$row_syear['syID']."'><span class='badge bg-success rounded-pill'><i class='fas fa-pencil-alt text-warning p-1'></i> Edit</span></a> <a href='#' class='delete_syear' data-id='".$row_syear['syID']."'><span class='badge bg-danger rounded-pill'><i class='fas fa-trash text-warning p-1'></i> Delete</span></a> <a href='?deactivate=".$row_syear['syID']."'><span class='badge bg-info rounded-pill'><i class='fas fa-times text-white p-1'></i> Deactivate</span></a>";
                                      }
                                    echo "</td>";
                                  echo "</tr>";
                                }
    ...
    <script>
      $(document).on('click', '.delete_syear', function(){
        var id = $(this).data('id');
        ...
        swal.fire({
            ...
            if (result.value){
              $.ajax({
                url: 'delete_function.php?action=delete_syear',
                type: 'POST',
                  data: 'id='+id,
                  dataType: 'json'
              })
              .done(function(response){
                Swal.fire({
                    ...
                    icon: 'success',
                    title: 'Success',
                    text: 'School year was successfully deleted',
                    footer: '',
              ...
              .fail(function(){
                Swal.fire({
                    ...
                    position: 'top-end',
                    title: 'Failed',
                    text: 'School year was not successfully deleted',
                    footer: '',
                    showConfirmButton: false,
                    timer: 2000
                  }).then(function() {
