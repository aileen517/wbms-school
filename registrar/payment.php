                                ...
                                while($row_student = mysqli_fetch_array($display_student)) {
                                  ...
                                  while($row_secname = mysqli_fetch_array($section_name)) {
                                    echo "<tr>";
                                      echo "<td>";
                                        $total_pay = 0;
                                        $grade_tuition = mysqli_query($conn, "SELECT * FROM $gradelevel WHERE glID='".$row_student['glevel']."'");
                                        while($row_tuition = mysqli_fetch_array($grade_tuition)) {
                                          $stud_payment = mysqli_query($conn, "SELECT * FROM $payment WHERE student_id='".$row_student['studID']."' AND syear='".$row_student['syear']."'");
                                          while($row_payment = mysqli_fetch_array($stud_payment)) {
                                            $total_pay += $row_payment['amount'];
                                          }
                                          if($total_pay>=$row_tuition['tuition']) {
                                            echo "<a href='student-details?student_id=".$row_student['studID']."&&lrnno=".$row_student['lrnno']."'>".ucfirst($row_student['sfname'])." ".ucfirst($row_student['slname'])."</a>";
                                          }
                                          else {
                                            echo "<a href='#' data-toggle='modal' data-target='#exampleModalCenter".$row_student['studID']."'>".ucfirst($row_student['sfname'])." ".ucfirst($row_student['slname'])."</a>";
                                          }
                                        }
                                      echo "</td>";
                                      echo "<td>";
                                      ...
                                      echo "<td>";
                                        $total_pay1 = 0;
                                        $grade_tuition1 = mysqli_query($conn, "SELECT * FROM $gradelevel WHERE glID='".$row_student['glevel']."'");
                                        while($row_tuition1 = mysqli_fetch_array($grade_tuition1)) {
                                          $stud_payment1 = mysqli_query($conn, "SELECT * FROM $payment WHERE student_id='".$row_student['studID']."' AND syear='".$row_student['syear']."'");
                                          while($row_payment1 = mysqli_fetch_array($stud_payment1)) {
                                            $total_pay1 += $row_payment1['amount'];
                                          }
                                          if($total_pay1>=$row_tuition1['tuition']) {
                                            echo "Paid";
                                          }
                                          else {
                                            echo "Unpaid";
